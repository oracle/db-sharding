...
resource "null_resource" "sdb_demo_setup" {
  ...
  provisioner "file" {
    content  = <<-EOF
      #! /bin/bash
      CSTR='(DESCRIPTION=(CONNECT_TIMEOUT=5)(TRANSPORT_CONNECT_TIMEOUT=3)(RETRY_COUNT=3)(ADDRESS_LIST=(LOAD_BALANCE=on)(ADDRESS=(PROTOCOL=TCP)(HOST=${oci_database_db_system.catalog_db[0].hostname}.${oci_database_db_system.catalog_db[0].domain})(PORT=${oci_database_db_system.catalog_db[0].listener_port})))(CONNECT_DATA=(SERVICE_NAME=${data.oci_database_database.catalog_database[0].db_unique_name}.${oci_database_db_system.catalog_db[0].domain})))'

      echo -e '@sql/1_dba_global_views.sql\n' | sqlplus -S sys/${var.sys_pass}@$CSTR as sysdba
  ...
  provisioner "file" {
    content  = <<-EOF
      connect app_schema/App_Schema_Pass_123@${oci_database_db_system.catalog_db[0].hostname}.${oci_database_db_system.catalog_db[0].domain}:${oci_database_db_system.catalog_db[0].listener_port}/${data.oci_database_database.catalog_database[0].pdb_name}.${oci_database_db_system.catalog_db[0].domain}
...
provisioner "file" {
 ...
 provisioner "file" {
    content = <<EOF
name=demo
connect_string=(ADDRESS_LIST=(LOAD_BALANCE=off)(FAILOVER=on)(ADDRESS=(HOST=${oci_core_instance.gsm_vm[0].private_ip})(PORT=${lookup(var.shard_directors[element(keys(var.shard_directors), 0)], "port")})(PROTOCOL=tcp)))
