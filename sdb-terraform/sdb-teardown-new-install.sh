...
function usage {
    echo "Script to teardown Oracle Sharded Database (SDB) with the following optional arguments"
    echo "usage: $scriptname [with-shard-standby] [with-catalog-standby] [with-demo]"
    echo "  with-shard-standby     Teardown SDB with shard standby"
...
then
    ...
    then
        terraform destroy -target null_resource.sdb_create_standby_shard_exec -var-file=terraform.tfvars -var-file=secrets.tfvars -auto-approve
        terraform destroy -target null_resource.sdb_create_standby_shard -var-file=terraform.tfvars -var-file=secrets.tfvars -auto-approve
    fi
    terraform destroy -target null_resource.sdb_create_shard_exec -var-file=terraform.tfvars -var-file=secrets.tfvars -auto-approve
    terraform destroy -target null_resource.sdb_create_shard -var-file=terraform.tfvars -var-file=secrets.tfvars -auto-approve
    if [ "$1" = "with-shard-standby" ] || [ "$2" = "with-shard-standby" ] || [ "$3" = "with-shard-standby" ]
    then
    ...
    if [ "$1" = "with-shard-standby" ] || [ "$2" = "with-shard-standby" ] || [ "$3" = "with-shard-standby" ]
    then
        terraform destroy -target null_resource.sdb_schagent_register_standby_shard -var-file=terraform.tfvars -var-file=secrets.tfvars -auto-approve
    fi
    terraform destroy -target null_resource.sdb_schagent_register_shard -var-file=terraform.tfvars -var-file=secrets.tfvars -auto-approve
