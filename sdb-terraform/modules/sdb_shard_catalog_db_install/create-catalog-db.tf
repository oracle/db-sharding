...
resource "null_resource" "sdb_shard_catalog_db_create" {
  # depends_on  = ["null_resource.sdb_shard_catalog_db_install_sw"]
...
# Mandatory     : Yes
#-----------------------------------------------------------------------------
gdbName=${lookup(var.shard_catalogs[element(keys(var.shard_catalogs), count.index)], "globalDBName")}
...
# Mandatory     : No
#-----------------------------------------------------------------------------
sid=${lookup(var.shard_catalogs[element(keys(var.shard_catalogs), count.index)], "sid")}
...
# Mandatory     : NO
#-----------------------------------------------------------------------------
variables=ORACLE_BASE_HOME=${var.db_home_path},DB_UNIQUE_NAME=${lookup(var.shard_catalogs[element(keys(var.shard_catalogs), count.index)], "sid")},ORACLE_BASE=${var.oracle_base},PDB_NAME=,DB_NAME=${lookup(var.shard_catalogs[element(keys(var.shard_catalogs), count.index)], "sid")},ORACLE_HOME=${var.db_home_path},SID=${lookup(var.shard_catalogs[element(keys(var.shard_catalogs), count.index)], "sid")}
...
EOF
  ...
  provisioner "file" {
    content  = "${data.template_file.shard_create_catalog_db_deps_check_template.rendered}" 
    destination = "${var.db_home_path}/shard-create-catalog-db-deps-check.sh"
  }
