...
######### Stop RAC DB ########
      def stop_rac_db(self,dbuser,dbhome,dbuname,hostname):
         ...
         cmd='''export ORACLE_HOME={0};export PATH={1};export LD_LIBRARY_PATH={2}; {0}/bin/srvctl stop database -d {3}'''.format(dbhome,path,ldpath,dbuname,hostname,dbuser)
         output,error,retcode=self.execute_cmd(cmd,None,None)
         self.check_os_err(output,error,retcode,True)
...
######### Stop RAC DB ########
      def start_rac_db(self,dbuser,dbhome,dbuname,node=None,startoption=None):
         ...
         cmd='''export ORACLE_HOME={0};export PATH={1};export LD_LIBRARY_PATH={2}; {0}/bin/srvctl start database -d {3} {6}'''.format(dbhome,path,ldpath,dbuname,nodename,dbuser,startflag)
         output,error,retcode=self.execute_cmd(cmd,None,None)
         self.check_os_err(output,error,retcode,True)
...
######### Sqlplus   ###########
      def get_inst_sid(self,dbuser,dbhome,osid,hostname):
         ...
         path='''/usr/bin:/bin:/sbin:/usr/local/sbin:{0}/bin'''.format(dbhome)
         ldpath='''{0}/lib:/lib:/usr/lib'''.format(dbhome)
         cmd='''export ORACLE_HOME={0};export PATH={1};export LD_LIBRARY_PATH={2}; {0}/bin/srvctl status database -d {3} | grep {4}'''.format(dbhome,path,ldpath,osid,hostname,dbuser)
         output,error,retcode=self.execute_cmd(cmd,None,None)
...
""".format(opdb)
         try:
             tns_file='''{0}/network/admin/tnsnames.ora'''.format(ohome)
             #cmd = f"ssh {node} 'echo \"{tns_entry_string}\" >> {tns_file}'"
             cmd = """ssh {0} 'echo \"{1}\" >> {2}'""".format(node, tns_entry_string, tns_file)
             subprocess.run(cmd, shell=True, check=True)
             self.log_info_message("Successfully added tns entry on {node}",self.file_name)
...
######### Return the all Cluster Nodes using olsnodes ##############
      def get_all_cls_nodes(self):
         ...
         cluster_nodes=None
         #cmd = f'sudo su - {giuser} -c "{gihome}/bin/olsnodes"'
