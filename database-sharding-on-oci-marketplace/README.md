...
Oracle Globally Distributed Database can be found on the featured list at the top of the OCI applications on Oracle public marketplace.
...
2. Goto the following listing link:  
...
1. Log in to the OCI console and click the left tooltips navigation menu.
2. Select <strong>OCI marketplace</strong> under <strong>Solutions and Marketplace</strong> in the left navigation menu.
...
5. Click the Oracle Database Sharding Tile. This will bring up the listing for Oracle Globally Distributed Database.
...
As specified in the system requirements section of the Oracle Globally Distributed Database listing, checkj the following prerequisites before deploying Oracle Globally Distributed Database. 

Because automatic and uniform distribution of Oracle Globally Distributed Database resources within a region is employed, Oracle recommends that you check to confirm you have sufficient number of all of the resources and services listed here in each availability domain within the region selected, before initial deployment or scaling of Oracle Globally Distributed Database.

The resources and services to check are the limits, quotas and availability, which are listed in the Governance section of the Tooltips menu on the left navigation pane. They include the following: 
...
After clicking <strong>Launch instance</strong> on the Oracle Globally Distributed Database Marketplace product page, Fill in the required Stack information.
...
* Name - Name of the Stack. The stack is given a default name and is provided with a date time stamp. You can edit this detail, if required.
* Description - Description that you provide while creating the Stack.
* Create In Compartment – The compartment defaults to the compartment that you selected on the Oracle Globally Distributed Database Marketplace page before clicking <strong>Launch stack</strong>.
* Terraform Version - The default is 0.12x.
...
* REPLICATION FACTOR - Choose a replicaton factor. The total number of shard databases provisioned will be equal to the number of primary shards that you select, multiplied by the chosen Replication Factor. The total number of primary catalog databases that will be provisioned will be equal to the Replication Factor chosen.
...
* CREATE NEW NETWORK - Select this to create a new regional network (recommended if this is your first sharded database) or Uncheck to use an existing regional network from a previously created sharded database
...
* SSH PUBLIC KEY - Public key for allowing SSH access to the Oracle Globally Distributed Database hosts.
...
* On the Review page, review the information that you provided, and then click <strong>Create</strong>.
...
* After stacks creation is complete, you can now view the output pane on the left for connection information to Oracle Globally Distributed Database.

## Oracle Globally Distributed Database Deployment job

After you click <strong>Create</strong> on the Review page, an Oracle Resource Manager (ORM) job is automatically created. This job starts deploying Oracle Globally Distributed Database.
...
Deployments with Replication Factor set to 1 take about 15 minutes. Deployments with Replication Factor set to 2 take about 45 minutes. When deployment is complete, the Oracle Globally Distributed Database deployment job should indicate a successful deployment as shown in the following examples:
...
![Oracle Globally Distributed Database ORM job success details](
...
All resources that are provisioned for the successful deployment of Oracle Globally Distributed Database are listed in the associated resources pane on the left menu within the job page. For example:
...
A _Shard Director_ is a compute that is a specfic implementation of a global service manager that acts as a regional listener for clients that connect to a sharded database. It can be directly accessed by clicking on the *sd* resource link.

1. Click on the shard director resource selected. For example:
...
A _Shard_, which is an Oracle Database, can be directly accessed by clicking on the *sh* resource link.

1. Click on the Shard resource selected. For example: 
...
The initial random credentials for Oracle Globally Distributed Database is available in the view state, which is securely stored within Oracle Resource Manager. Oracle recommends that you change these credentials on your first attempt to use these initial random passwords.
...
* Click on the left tooltips navigation Menu. Choose <strong>Resource Manager</strong> under <strong>Solutions and Marketplace</strong>.
...
Shards can be _scaled in_ (decreased) or _scaled out_ (decreased), based on your requirements and as needed. You can scale shards without affecting the performance of queries per second (qps) or transactions per second (tps), and with zero downtime.
...
After clicking <strong>Apply job</strong>, a confirmation dialog pops up confirm that you want to apply the changes to the existing sharded database deployment.
...
![Oracle Globally Distributed Database edit stack apply job running](
...
Depending upon the changes requested in the sharding edit stack, the apply job will make the necessary changes in the sharded database resources. After the changes are successfully applied, the job status indicates they are complete. For example:
...
The apply job details shows the the changes that are made to the sharded database. For example: 
...
After the edit stack apply job has completed successfully, the outputs section on the left pane of the apply job page contains the details on how to access the Oracle Globally Distributed Database. For example:
...
Shard Directors can be _scaled in_ (decreased) or _scaled out_ (decreased) based as needed on your requirements. You can scale shard directors without affecting performance and with zero downtime.

The steps for scaling are the same as scaling shards as shown in the previous section. Before saving changes and running an apply job to deploy the changes, Change the shard director configuration, as shown in this example:
...
To terminate an Oracle Globally Distributed Database created from OCI marketplace in the past, follow these steps:

* Click the left tooltips Menu. Choose <stong>Resource Manager</strong> under <strong>Solutions and Marketplace</strong>.
* Select the stack that you created from Narketplace.
* Click <strong>Terraform Actions</strong>, and then <strong>Destroy</strong>. After you click this, all Sharded database resources including any user data will be deleted from OCI. 
...
For more details on Oracle Globally Distributed Database and usage, see: [Product page](https://www.oracle.com/database/technologies/high-availability/sharding.html). 

For product documentation, see: [product release documentation](https://docs.oracle.com/en/database/oracle/oracle-database/19/shard/sharding-overview.html)

For GDSCTL CLI commands to manage sharded database manually after they are deployed on OCI, see: [Sharding GDSCTL CLI command reference](https://docs.oracle.com/en/database/oracle/oracle-database/19/gsmug/gdsctl-sharding-env.html)  